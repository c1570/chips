typedef struct {
    uint16_t address;
    const char *name;
    int flag;
} c1541_routine_t;

const c1541_routine_t c1541_routines[] = {
    {0xFF20, "PATCH_WAIT_FOR_DATA_HI_INACTIVE", 1},
    {0xFF15, "PATCH_CLEAR_ATNA_CLK_SET_DATA", 1},
    {0xFF0D, "NNMI10", 1},
    {0xFF01, "NNMI", 1},
    {0xFEF3, "SLOWD", 1},
    {0xFEEA, "PEA7A", 1},
    {0xFEE7, "NMI", 1},
    {0xFE7F, "IRQ20", 1},
    {0xFE76, "IRQ10", 1},
    {0xFE67, "SYSIRQ", 1},
    {0xFE44, "FBG10", 1},
    {0xFE30, "FBTOG", 1},
    {0xFE26, "CLER10", 1},
    {0xFE0E, "CLEAR", 1},
    {0xFE00, "KILL", 1},
    {0xFDF5, "MOVOVR", 1},
    {0xFDE5, "MOVUP", 1},
    {0xFDDB, "FMTE10", 1},
    {0xFDD3, "FMTERR", 1},
    {0xFDC9, "WRTN10", 1},
    {0xFDC3, "WRTNUM", 1},
    {0xFDB9, "SYC10", 1},
    {0xFDA3, "SYNCLR", 1},
    {0xFD96, "FMTEND", 1},
    {0xFD77, "TST10", 1},
    {0xFD67, "TST05", 1},
    {0xFD62, "TSTDAT", 1},
    {0xFD58, "CMPR20", 1},
    {0xFD40, "CMPR15", 1},
    {0xFD39, "CMPR10", 1},
    {0xFD2C, "COMP", 1},
    {0xFD09, "WGP2", 1},
    {0xFCF9, "WRTS50", 1},
    {0xFCEB, "WRTS40", 1},
    {0xFCE0, "DBSYNC", 1},
    {0xFCD1, "WRTS30", 1},
    {0xFCC2, "WRTS20", 1},
    {0xFCB8, "WRTS10", 1},
    {0xFCB1, "WRTSYN", 1},
    {0xFC86, "CRTDAT", 1},
    {0xFC3F, "MAK10", 1},
    {0xFBE0, "DS08", 1},
    {0xFBCE, "CNT20", 1},
    {0xFBBB, "CNT10", 1},
    {0xFBB6, "COUNT", 1},
    {0xFB7D, "F009", 1},
    {0xFB67, "F007", 1},
    {0xFB64, "F006", 1},
    {0xFB5C, "F005", 1},
    {0xFB46, "F001", 1},
    {0xFB43, "F000", 1},
    {0xFB3E, "FWAIT2", 1},
    {0xFB39, "FWAIT", 1},
    {0xFB0C, "TOPP", 1},
    {0xFB00, "L214", 1},
    {0xFAF5, "L213", 1},
    {0xFAC7, "FORMT", 1},
    {0xFABE, "END33", 1},
    {0xFAA5, "SSDEC", 1},
    {0xFA97, "SSRUN", 1},
    {0xFA94, "SSA10", 1},
    {0xFA7B, "SSACL", 1},
    {0xFA69, "STP", 1},
    {0xFA63, "STPIN", 1},
    {0xFA4E, "SETLE", 1},
    {0xFA3B, "SHORT", 1},
    {0xFA32, "STPOUT", 1},
    {0xFA2E, "DOSTEP", 1},
    {0xFA1C, "INAC20", 1},
    {0xFA0E, "INAC10", 1},
    {0xFA05, "INACT", 1},
    {0xF9FA, "END30", 1},
    {0xF9E4, "END20", 1},
    {0xF9D9, "END10", 1},
    {0xF9D6, "END33X", 1},
    {0xF99C, "END", 1},
    {0xF98F, "TRNOFF", 1},
    {0xF97E, "TURNON", 1},
    {0xF975, "ERRR10", 1},
    {0xF969, "ERRR", 1},
    {0xF934, "CONHDR", 1},
    {0xF92B, "GCRB20", 1},
    {0xF90C, "GCRB10", 1},
    {0xF8E0, "GCRBIN", 1},
    {0xF7E6, "GET4GB", 1},
    {0xF7BA, "BING07", 1},
    {0xF78F, "BINGCR", 1},
    {0xF77F, "BGTAB", 1},
    {0xF6D0, "PUT4GB", 1},
    {0xF6CA, "SECTSK", 1},
    {0xF6C5, "VRF20", 1},
    {0xF6B3, "VRF30", 1},
    {0xF6A3, "VRF15", 1},
    {0xF691, "VRFY", 1},
    {0xF683, "WTOB57", 1},
    {0xF66E, "WTOB52", 1},
    {0xF64F, "WTOB53", 1},
    {0xF643, "WTOB50", 1},
    {0xF624, "WTOB14", 1},
    {0xF5F2, "WTOBIN", 1},
    {0xF5E9, "CHKBLK", 1},
    {0xF5BF, "WRT40", 1},
    {0xF5B3, "WRT30", 1},
    {0xF5AB, "WRTSNC", 1},
    {0xF586, "WRT10", 1},
    {0xF56E, "WRIGHT", 1},
    {0xF55D, "SYNC10", 1},
    {0xF556, "SYNC", 1},
    {0xF553, "ERROR_TIMEOUT_WAITING_FOR_SYNC", 1},
    {0xF54E, "SRCH30", 1},
    {0xF53D, "SRCH25", 1},
    {0xF538, "SRCH20", 1},
    {0xF510, "SRCH", 1},
    {0xF50A, "DSTRT", 1},
    {0xF4FB, "READ28", 1},
    {0xF4DF, "READ20", 1},
    {0xF4D4, "READ11", 1},
    {0xF4D1, "READ01", 1},
    {0xF4CA, "READ", 1},
    {0xF497, "CNVRTN", 1},
    {0xF483, "L470", 1},
    {0xF47E, "TSTRDJ", 1},
    {0xF473, "DOITT", 1},
    {0xF461, "L465", 1},
    {0xF43A, "L480", 1},
    {0xF432, "L460", 1},
    {0xF423, "WSECT", 1},
    {0xF41E, "CSERR", 1},
    {0xF41B, "BADID", 1},
    {0xF418, "DONE", 1},
    {0xF410, "ESEEK", 1},
    {0xF407, "SEEK20", 1},
    {0xF3D8, "SEEK30", 1},
    {0xF3C8, "SEEK15", 1},
    {0xF3B1, "SEAK", 1},
    {0xF393, "SETJB", 1},
    {0xF37C, "BMP", 1},
    {0xF377, "EX", 1},
    {0xF367, "EXE", 1},
    {0xF33C, "GOTU", 1},
    {0xF320, "QUE20", 1},
    {0xF306, "QUE05", 1},
    {0xF2F9, "QUE", 1},
    {0xF2F3, "CONT20", 1},
    {0xF2E9, "CONT40", 1},
    {0xF2D8, "CONT35", 1},
    {0xF2CD, "CONT30", 1},
    {0xF2C3, "CONT10", 1},
    {0xF2BE, "TOP", 1},
    {0xF2B0, "LCC", 1},
    {0xF259, "CNTINT", 1},
    {0xF258, "KILLP", 1},
    {0xF24E, "MAX1", 1},
    {0xF24B, "MAXSEC", 1},
    {0xF246, "AC40", 1},
    {0xF236, "AC30", 1},
    {0xF22D, "AC20", 1},
    {0xF22B, "AC10", 1},
    {0xF220, "AVCK", 1},
    {0xF21F, "GS30", 1},
    {0xF21D, "GS20", 1},
    {0xF20D, "GS10", 1},
    {0xF1FA, "GETSEC", 1},
    {0xF1F5, "DERR", 1},
    {0xF1E6, "FNDSEC", 1},
    {0xF1DF, "ITS3", 1},
    {0xF1CB, "ITS2", 1},
    {0xF1A9, "INTTS", 1},
    {0xF19D, "FNDN2", 1},
    {0xF19A, "FNDN1", 1},
    {0xF195, "FNDN0", 1},
    {0xF173, "FNDNXT", 1},
    {0xF15F, "NXT2", 1},
    {0xF15A, "NXTERR", 1},
    {0xF130, "NXT1", 1},
    {0xF12D, "NXTDS", 1},
    {0xF11E, "NXTTS", 1},
    {0xF119, "BAAM2X", 1},
    {0xF118, "B2X10", 1},
    {0xF10F, "BAM2A", 1},
    {0xF10A, "RBM20", 1},
    {0xF0F2, "RBM10", 1},
    {0xF0DF, "REDBAM", 1},
    {0xF0D1, "CLNBAM", 1},
    {0xF0D0, "SWAP2", 1},
    {0xF0BE, "SWAP1", 1},
    {0xF0A5, "PUTBAM", 1},
    {0xF09F, "SWAP4", 1},
    {0xF07F, "SWAP3", 1},
    {0xF05B, "SWAP", 1},
    {0xF03E, "SBM30", 1},
    {0xF022, "SBM10", 1},
    {0xF011, "SETBAM", 1},
    {0xF00B, "CLB1", 1},
    {0xF005, "CLRBAM", 1},
    {0xF004, "FBAM10", 1},
    {0xEFF1, "FIXBAM", 1},
    {0xEFE9, "BMASK", 1},
    {0xEFD5, "FREUS3", 1},
    {0xEFD3, "FREUS2", 1},
    {0xEFCF, "FREUSE", 1},
    {0xEFCE, "USERTS", 1},
    {0xEFBD, "USE20", 1},
    {0xEFBA, "USE10", 1},
    {0xEF93, "USEDTS", 1},
    {0xEF90, "WUSED", 1},
    {0xEF88, "DTYBAM", 1},
    {0xEF87, "FRERTS", 1},
    {0xEF62, "FRETS2", 1},
    {0xEF5F, "FRETS", 1},
    {0xEF5C, "WFREE", 1},
    {0xEF4D, "NUMFRE", 1},
    {0xEF3A, "SETBPT", 1},
    {0xEF24, "SB20", 1},
    {0xEF07, "SB10", 1},
    {0xEEFF, "SCRBAM", 1},
    {0xEEF4, "MAPOUT", 1},
    {0xEEE3, "NM30", 1},
    {0xEED9, "NM20", 1},
    {0xEEC7, "NM10", 1},
    {0xEEB7, "NEWMAP", 1},
    {0xEE56, "N110", 1},
    {0xEE46, "N108", 1},
    {0xEE19, "N101", 1},
    {0xEE0D, "NEW", 1},
    {0xEE04, "MRK1", 1},
    {0xEDEE, "MRK2", 1},
    {0xEDE5, "VMKBAM", 1},
    {0xEDD9, "VD25", 1},
    {0xEDD4, "VD20", 1},
    {0xEDCB, "VD17", 1},
    {0xEDB3, "VD15", 1},
    {0xED9C, "VD10", 1},
    {0xED84, "VERDIR", 1},
    {0xED7E, "GD1", 1},
    {0xED6D, "GETD3", 1},
    {0xED67, "GETDIR", 1},
    {0xED5B, "MOVB1", 1},
    {0xED59, "MOVBUF", 1},
    {0xED23, "DIR3", 1},
    {0xED0D, "DIR10", 1},
    {0xECEA, "DIR1", 1},
    {0xEC9E, "STDIR", 1},
    {0xEC98, "IDL12", 1},
    {0xEC8B, "IDL11", 1},
    {0xEC81, "IDL10", 1},
    {0xEC6D, "IDL9", 1},
    {0xEC69, "IDL8", 1},
    {0xEC5C, "IDL7", 1},
    {0xEC58, "IDL6", 1},
    {0xEC3B, "IDL5", 1},
    {0xEC31, "IDL4", 1},
    {0xEC2B, "IDL3", 1},
    {0xEC12, "IDL02", 1},
    {0xEC07, "IDL01", 1},
    {0xEBFF, "IDL1", 1},
    {0xEBED, "IDLE_CLEAR_ATNA_DATA", 1},
    {0xEBE7, "IDLE", 1},
    {0xEBDC, "SETERR_CLEAR_ATNA_CLK_DATA", 1},
    {0xEBD5, "SETERR", 1},
    {0xEB7E, "DSKIN2", 1},
    {0xEB76, "DSKIN1", 1},
    {0xEB4F, "INTT1", 1},
    {0xEB4B, "INTTAB", 1},
    {0xEB22, "DIAGOK", 1},
    {0xEB1F, "PERR2", 1},
    {0xEB04, "RA40", 1},
    {0xEB02, "RA30", 1},
    {0xEAF2, "RA10", 1},
    {0xEAF0, "RAMTST", 1},
    {0xEAEC, "CR30", 1},
    {0xEAEA, "CR20", 1},
    {0xEAD7, "RT20", 1},
    {0xEAD5, "RT10", 1},
    {0xEAC9, "RM10", 1},
    {0xEAB7, "PV30", 1},
    {0xEAB2, "PV20", 1},
    {0xEAAC, "PV10", 1},
    {0xEAA0, "DSKINT", 1},
    {0xEA90, "PD21", 1},
    {0xEA8F, "PD11", 1},
    {0xEA8E, "PE40", 1},
    {0xEA7F, "PD20", 1},
    {0xEA7E, "PD10", 1},
    {0xEA7D, "REA7D", 1},
    {0xEA75, "PE30", 1},
    {0xEA74, "PE20", 1},
    {0xEA71, "PERR", 1},
    {0xEA6E, "PEZRO", 1},
    {0xEA6B, "TATN20", 1},
    {0xEA63, "TSTA50", 1},
    {0xEA62, "TSTRTN", 1},
    {0xEA59, "TSTATN", 1},
    {0xEA50, "RELEASEALL_CLEAR_ATNA_CLK_DATA", 1},
    {0xEA4E, "RELEASEALL", 1},
    {0xEA44, "LSN30", 1},
    {0xEA39, "LSN15", 1},
    {0xEA2E, "LISTEN", 1},
    {0xEA1A, "ACP03A", 1},
    {0xEA0B, "ACP03", 1},
    {0xEA07, "ACP01", 1},
    {0xE9FD, "ACP02A", 1},
    {0xE9F2, "ACPOOB", 1},
    {0xE9DF, "ACP00", 1},
    {0xE9CD, "ACP00A", 1},
    {0xE9C9, "ACPTR", 1},
    {0xE9C0, "DEBNC", 1},
    {0xE9B7, "SET_CLK_HI_INACTIVE", 1},
    {0xE9AE, "SET_CLK_LOW_ACTIVE", 1},
    {0xE9A5, "SET_DATA_LOW_ACTIVE", 1},
    {0xE99C, "SET_DATA_HI_INACTIVE", 1},
    {0xE999, "FRMERX", 1},
    {0xE987, "ISR04", 1},
    {0xE980, "ISR03", 1},
    {0xE976, "ISRCLK", 1},
    {0xE973, "ISRHI", 1},
    {0xE963, "ISR02", 1},
    {0xE95C, "ISR01", 1},
    {0xE94B, "NOEOI", 1},
    {0xE941, "TLK03", 1},
    {0xE937, "TLK02", 1},
    {0xE925, "TALK2", 1},
    {0xE916, "TLK05", 1},
    {0xE915, "NOTLK", 1},
    {0xE90F, "TALK1", 1},
    {0xE909, "TALK", 1},
    {0xE902, "ATN122", 1},
    {0xE8FF, "ATN120_SET_ATNA_CLEAR_CLK_DATA", 1},
    {0xE8FD, "ATN120", 1},
    {0xE8FA, "ATN110", 1},
    {0xE8ED, "ATN100", 1},
    {0xE8E0, "ATSN20_CLEAR_ATNA", 1},
    {0xE8D7, "ATSN20", 1},
    {0xE8D2, "ATN95", 1},
    {0xE8B7, "ATN50", 1},
    {0xE8A9, "ATN45", 1},
    {0xE89B, "ATN40", 1},
    {0xE891, "ATN35", 1},
    {0xE87B, "ATNS15", 1},
    {0xE878, "ATNSRV_SET_ATNA", 1},
    {0xE85B, "ATNSRV", 1},
    {0xE853, "ATNIRQ", 1},
    {0xE84B, "ADDSUM", 1},
    {0xE848, "GTABYE", 1},
    {0xE839, "GTABYT", 1},
    {0xE82C, "UTLD50", 1},
    {0xE817, "UTLD35", 1},
    {0xE802, "UTLD30", 1},
    {0xE7FA, "UTLD20", 1},
    {0xE7D8, "UTLD10", 1},
    {0xE7C5, "UTLD00", 1},
    {0xE7A8, "BOOT4", 1},
    {0xE7A3, "UTLODR", 1},
    {0xE78E, "BOOT3", 1},
    {0xE780, "BOOT", 1},
    {0xE77F, "BOOT2", 1},
    {0xE77E, "EA20", 1},
    {0xE775, "EADV2", 1},
    {0xE76D, "EA10", 1},
    {0xE767, "EADV1", 1},
    {0xE763, "E70", 1},
    {0xE754, "E60", 1},
    {0xE74D, "E90", 1},
    {0xE742, "E55", 1},
    {0xE73D, "E50", 1},
    {0xE739, "E45", 1},
    {0xE735, "E40", 1},
    {0xE727, "E30", 1},
    {0xE722, "E20", 1},
    {0xE718, "E10", 1},
    {0xE706, "ERMOVE", 1},
    {0xE6C7, "ERRMSG", 1},
    {0xE6C1, "ERRTSO", 1},
    {0xE6BC, "OKERR", 1},
    {0xE6B4, "BCD2", 1},
    {0xE6AB, "BCDDEC", 1},
    {0xE6AA, "HEX5", 1},
    {0xE69F, "HEX0", 1},
    {0xE69B, "HEXDEC", 1},
    {0xE698, "ERR10", 1},
    {0xE68E, "TLERR", 1},
    {0xE688, "LSNERR", 1},
    {0xE680, "TLKERR", 1},
    {0xE648, "CMDER3", 1},
    {0xE645, "CMDER2", 1},
    {0xE644, "ERR4", 1},
    {0xE62D, "ERR3", 1},
    {0xE627, "ERR2", 1},
    {0xE625, "ERR1", 1},
    {0xE60A, "ERROR", 1},
    {0xE4DE, "NS50", 1},
    {0xE4D1, "NS40", 1},
    {0xE4AC, "NS20", 1},
    {0xE44E, "NEWSS", 1},
    {0xE444, "AR70", 1},
    {0xE418, "AR65", 1},
    {0xE3F9, "AR60", 1},
    {0xE3D4, "AR55", 1},
    {0xE3C8, "AR50", 1},
    {0xE3B6, "AR45", 1},
    {0xE39D, "AR40", 1},
    {0xE38F, "AR35", 1},
    {0xE372, "AR30", 1},
    {0xE368, "AR25", 1},
    {0xE363, "AR20", 1},
    {0xE355, "AR10", 1},
    {0xE33B, "ADDR1", 1},
    {0xE31C, "ADDREL", 1},
    {0xE31B, "AN10", 1},
    {0xE318, "AN05", 1},
    {0xE304, "ADDNR", 1},
    {0xE303, "NB30", 1},
    {0xE2F1, "NB20", 1},
    {0xE2E2, "NULBUF", 1},
    {0xE2DC, "BH10", 1},
    {0xE2D3, "BHERE2", 1},
    {0xE2D0, "BHERE", 1},
    {0xE2C2, "P80", 1},
    {0xE2BF, "P75", 1},
    {0xE2AA, "P10", 1},
    {0xE29C, "POSBUF", 1},
    {0xE291, "P30", 1},
    {0xE289, "P2", 1},
    {0xE275, "POSITN", 1},
    {0xE272, "R60", 1},
    {0xE265, "R50", 1},
    {0xE253, "R40", 1},
    {0xE228, "R30", 1},
    {0xE219, "R20", 1},
    {0xE207, "RECORD", 1},
    {0xE202, "BREAK", 1},
    {0xE1EF, "SE30", 1},
    {0xE1DC, "SE20", 1},
    {0xE1D8, "SE10", 1},
    {0xE1CE, "SSEND", 1},
    {0xE1C8, "FNDL20", 1},
    {0xE1C4, "FNDL30", 1},
    {0xE1B7, "FNDL10", 1},
    {0xE1B2, "FNDLST", 1},
    {0xE1AC, "SETL40", 1},
    {0xE1A4, "SETL10", 1},
    {0xE19D, "SETL05", 1},
    {0xE17E, "SETL01", 1},
    {0xE16E, "SETLST", 1},
    {0xE15E, "RD05", 1},
    {0xE153, "RD40", 1},
    {0xE14D, "RD30", 1},
    {0xE13D, "RD25", 1},
    {0xE13B, "RD20", 1},
    {0xE138, "RD15", 1},
    {0xE127, "RD10", 1},
    {0xE120, "RDREL", 1},
    {0xE115, "CDIRTY", 1},
    {0xE105, "SDIRTY", 1},
    {0xE104, "CLR10", 1},
    {0xE0F3, "CLREC", 1},
    {0xE0E2, "WR60", 1},
    {0xE0E1, "WR51", 1},
    {0xE0D9, "WR50", 1},
    {0xE0D6, "WR45", 1},
    {0xE0C8, "WR40", 1},
    {0xE0BC, "WR30", 1},
    {0xE0B7, "WR20", 1},
    {0xE0B2, "WR10", 1},
    {0xE0AB, "WRTREL", 1},
    {0xE0AA, "RELP20", 1},
    {0xE0A3, "RELP10", 1},
    {0xE09E, "RELP07", 1},
    {0xE096, "RELP05", 1},
    {0xE094, "RELP06", 1},
    {0xE07C, "RELPUT", 1},
    {0xE07B, "NRBU20", 1},
    {0xE06B, "NRBU70", 1},
    {0xE05D, "NRBU50", 1},
    {0xE03C, "NRBUF", 1},
    {0xE035, "NXTR35", 1},
    {0xE034, "NXTR30", 1},
    {0xE02A, "NXTR50", 1},
    {0xE01D, "NXTR40", 1},
    {0xE018, "NXTR45", 1},
    {0xE009, "NXOUT", 1},
    {0xDFF6, "NXTR20", 1},
    {0xDFE4, "NXTR15", 1},
    {0xDFD0, "NXTREC", 1},
    {0xDFCD, "PI1", 1},
    {0xDFC2, "PUTINA", 1},
    {0xDFBF, "GI10", 1},
    {0xDFB7, "GETINA", 1},
    {0xDFB0, "GA3", 1},
    {0xDFA0, "GA2", 1},
    {0xDF9E, "GAFLGS", 1},
    {0xDF9B, "GA1", 1},
    {0xDF93, "GETACT", 1},
    {0xDF8F, "ST40", 1},
    {0xDF8B, "ST30", 1},
    {0xDF7B, "ST20", 1},
    {0xDF77, "ST10", 1},
    {0xDF66, "SSTEST", 1},
    {0xDF65, "ADDRTS", 1},
    {0xDF5C, "ADDT12", 1},
    {0xDF51, "SSCALC", 1},
    {0xDF4C, "SCAL1", 1},
    {0xDF45, "GSSPNT", 1},
    {0xDF25, "IBOP", 1},
    {0xDF21, "IBWT", 1},
    {0xDF1B, "IBRD", 1},
    {0xDF12, "SSP20", 1},
    {0xDF0B, "SSP10", 1},
    {0xDEF8, "SSPOS", 1},
    {0xDEE9, "SETSSP", 1},
    {0xDEDC, "SSDIR", 1},
    {0xDED2, "SSSET", 1},
    {0xDECC, "CB10", 1},
    {0xDEC1, "CLRBUF", 1},
    {0xDEB6, "B02", 1},
    {0xDEA5, "B0TOB0", 1},
    {0xDE95, "RDLNK", 1},
    {0xDE8B, "SJ20", 1},
    {0xDE7F, "SJ10", 1},
    {0xDE75, "RDS5", 1},
    {0xDE73, "RDSS", 1},
    {0xDE6C, "WRTSS", 1},
    {0xDE65, "RDIN", 1},
    {0xDE5E, "WRTOUT", 1},
    {0xDE57, "RDAB", 1},
    {0xDE50, "WRTAB", 1},
    {0xDE3E, "GETHDR", 1},
    {0xDE3B, "CURBLK", 1},
    {0xDE2B, "SET00", 1},
    {0xDE19, "NULLNK", 1},
    {0xDE0C, "GETLNK", 1},
    {0xDDFD, "SETLNK", 1},
    {0xDDFC, "SCR1", 1},
    {0xDDF1, "SCRUB", 1},
    {0xDDCA, "TSTC40", 1},
    {0xDDC9, "TSTRTS", 1},
    {0xDDC2, "TSTC30", 1},
    {0xDDB9, "TSTC20", 1},
    {0xDDB7, "TSTCHN", 1},
    {0xDDAB, "TSTWRT", 1},
    {0xDDA6, "TSTFLG", 1},
    {0xDDA3, "CLRF10", 1},
    {0xDD9D, "CLRFLG", 1},
    {0xDD97, "SETFLG", 1},
    {0xDD95, "SCFLG", 1},
    {0xDD8D, "PUTSS", 1},
    {0xDD16, "OW20", 1},
    {0xDCFD, "OW10", 1},
    {0xDCDA, "OPNWCH", 1},
    {0xDCB6, "INITP", 1},
    {0xDCA9, "OR30", 1},
    {0xDC98, "OROW", 1},
    {0xDC81, "OR20", 1},
    {0xDC65, "OR10", 1},
    {0xDC46, "OPNRCH", 1},
    {0xDC29, "CLSD6", 1},
    {0xDC21, "CLSD5", 1},
    {0xDC06, "CLSD4", 1},
    {0xDBA5, "CLSDIR", 1},
    {0xDB8C, "CLSW20", 1},
    {0xDB88, "CLSW15", 1},
    {0xDB76, "CLSW10", 1},
    {0xDB62, "CLSWRT", 1},
    {0xDB5F, "CLSR1", 1},
    {0xDB2C, "CLSREL", 1},
    {0xDB29, "CLSC31", 1},
    {0xDB26, "CLSC30", 1},
    {0xDB0C, "CLSC28", 1},
    {0xDB02, "CLSCHN", 1},
    {0xDAFF, "CLS25", 1},
    {0xDAF0, "CLS20", 1},
    {0xDAEC, "CLSALL", 1},
    {0xDAE9, "CLS15", 1},
    {0xDAD4, "CLS10", 1},
    {0xDAD1, "CLS05", 1},
    {0xDAC0, "CLOSE", 1},
    {0xDAA7, "LD20", 1},
    {0xDA9E, "LD10", 1},
    {0xDA90, "LD05", 1},
    {0xDA86, "LD03", 1},
    {0xDA6D, "LD02", 1},
    {0xDA62, "LD01", 1},
    {0xDA55, "LOADIR", 1},
    {0xDA42, "AP30", 1},
    {0xDA2A, "APPEND", 1},
    {0xDA29, "CKT2", 1},
    {0xDA1C, "CKM2", 1},
    {0xDA11, "CKM1", 1},
    {0xDA09, "CKTM", 1},
    {0xDA06, "OPF1", 1},
    {0xD9EF, "OPFIN", 1},
    {0xD9E3, "OPWRIT", 1},
    {0xD9C3, "OP130", 1},
    {0xD9A0, "OPREAD", 1},
    {0xD990, "OP125", 1},
    {0xD96A, "OP120", 1},
    {0xD965, "OP115", 1},
    {0xD95C, "OP110", 1},
    {0xD94A, "OP100", 1},
    {0xD945, "OP95", 1},
    {0xD940, "OP90", 1},
    {0xD8F5, "OP82", 1},
    {0xD8F0, "OP815", 1},
    {0xD8E1, "OP81", 1},
    {0xD8D9, "OP80", 1},
    {0xD8CD, "OP77", 1},
    {0xD8C6, "OP75", 1},
    {0xD8B1, "OP60", 1},
    {0xD8A7, "OP50", 1},
    {0xD891, "OP45", 1},
    {0xD876, "OP40", 1},
    {0xD840, "OP20", 1},
    {0xD83C, "OP10", 1},
    {0xD837, "OP05", 1},
    {0xD834, "OP049", 1},
    {0xD82B, "OP042", 1},
    {0xD81C, "OP0415", 1},
    {0xD815, "OP041", 1},
    {0xD7FF, "OP04", 1},
    {0xD7F3, "OP021", 1},
    {0xD7EB, "ENDRD", 1},
    {0xD7CF, "OP02", 1},
    {0xD7B4, "OPEN", 1},
    {0xD790, "AF50", 1},
    {0xD74D, "AF25", 1},
    {0xD73D, "AF20", 1},
    {0xD730, "AF15", 1},
    {0xD726, "AF10", 1},
    {0xD715, "AF08", 1},
    {0xD6E4, "ADDFIL", 1},
    {0xD6D0, "SETHDR", 1},
    {0xD6C4, "DOREC3", 1},
    {0xD6B9, "DOREC2", 1},
    {0xD6AB, "DOREC1", 1},
    {0xD6A6, "DOREC", 1},
    {0xD69A, "MH10", 1},
    {0xD693, "MCVHED", 1},
    {0xD692, "HOF3", 1},
    {0xD688, "HOF2", 1},
    {0xD67C, "HOF1", 1},
    {0xD676, "HEDOFF", 1},
    {0xD66D, "REC95", 1},
    {0xD65C, "REC9", 1},
    {0xD651, "REC8", 1},
    {0xD645, "REC5", 1},
    {0xD644, "REC7", 1},
    {0xD63F, "QUIT2", 1},
    {0xD635, "QUIT", 1},
    {0xD631, "REC5", 1},
    {0xD625, "REC3", 1},
    {0xD600, "REC1", 1},
    {0xD5F4, "REC0", 1},
    {0xD5E3, "REC01", 1},
    {0xD5C6, "RECOV", 1},
    {0xD5C4, "NOTYET", 1},
    {0xD5C2, "OK", 1},
    {0xD5BA, "TJ10", 1},
    {0xD5A6, "TSTJOB", 1},
    {0xD599, "WATJOB", 1},
    {0xD593, "DOIT2", 1},
    {0xD590, "DOIT", 1},
    {0xD58C, "DOJOB", 1},
    {0xD58A, "DOWRIT", 1},
    {0xD586, "DOREAD", 1},
    {0xD57A, "SJB1", 1},
    {0xD572, "VNERR", 1},
    {0xD55F, "TSCHK", 1},
    {0xD552, "HED2TS", 1},
    {0xD54D, "TSER1", 1},
    {0xD54A, "TSERR", 1},
    {0xD53F, "SJB4", 1},
    {0xD538, "SJB3", 1},
    {0xD535, "SJB2", 1},
    {0xD50E, "SETJOB", 1},
    {0xD506, "SETLJB", 1},
    {0xD4F6, "DRDBYT", 1},
    {0xD4EB, "SETDIR", 1},
    {0xD4E8, "GETPNT", 1},
    {0xD4DA, "FREICH", 1},
    {0xD4C8, "SETPNT", 1},
    {0xD4BB, "NXDB1", 1},
    {0xD48D, "NXDRBK", 1},
    {0xD486, "OPNIWR", 1},
    {0xD477, "OPNTYP", 1},
    {0xD475, "OPNIRD", 1},
    {0xD466, "DRT", 1},
    {0xD464, "DRTWRT", 1},
    {0xD460, "DRTRD", 1},
    {0xD45F, "NXTB1", 1},
    {0xD44D, "NXTBUF", 1},
    {0xD445, "GE30", 1},
    {0xD443, "GE20", 1},
    {0xD43A, "GE15", 1},
    {0xD433, "GE10", 1},
    {0xD414, "GETERC", 1},
    {0xD409, "GET6", 1},
    {0xD403, "GET3", 1},
    {0xD400, "SEQGET", 1},
    {0xD3FF, "RNGET3", 1},
    {0xD3F0, "RNGET4", 1},
    {0xD3EE, "RNGET2", 1},
    {0xD3EC, "RNGET1", 1},
    {0xD3DE, "RNDGET", 1},
    {0xD3D7, "GET2", 1},
    {0xD3D3, "GET1", 1},
    {0xD3CE, "GET0", 1},
    {0xD3B4, "GET00", 1},
    {0xD3AA, "GET", 1},
    {0xD39B, "GBYTE", 1},
    {0xD391, "FND30", 1},
    {0xD383, "FND10", 1},
    {0xD37F, "FNDLDX", 1},
    {0xD37A, "STL60", 1},
    {0xD373, "STL50", 1},
    {0xD363, "STL40", 1},
    {0xD35E, "STL30", 1},
    {0xD355, "STL20", 1},
    {0xD348, "STL10", 1},
    {0xD33E, "STL05", 1},
    {0xD339, "STLBUF", 1},
    {0xD317, "CLSD", 1},
    {0xD313, "CLDCHN", 1},
    {0xD30B, "CLRC1", 1},
    {0xD307, "CLRCHN", 1},
    {0xD2F9, "FREB1", 1},
    {0xD2F3, "FREBUF", 1},
    {0xD2E9, "FRI10", 1},
    {0xD2DA, "FREIAC", 1},
    {0xD2D9, "FRI20", 1},
    {0xD2D8, "FB3", 1},
    {0xD2C8, "FB2", 1},
    {0xD2BC, "FB1", 1},
    {0xD2BA, "FNDBUF", 1},
    {0xD2B6, "GBF2", 1},
    {0xD2A3, "GBF1", 1},
    {0xD28E, "GETBUF", 1},
    {0xD28D, "REL3", 1},
    {0xD27C, "REL2", 1},
    {0xD26B, "REL1", 1},
    {0xD25A, "RELBUF", 1},
    {0xD259, "FRE25", 1},
    {0xD253, "REL10", 1},
    {0xD24D, "REL15", 1},
    {0xD249, "RELINX", 1},
    {0xD22E, "FRECO", 1},
    {0xD227, "FRECHN", 1},
    {0xD226, "GETR4", 1},
    {0xD217, "GETR5", 1},
    {0xD20F, "GETERR", 1},
    {0xD206, "GETR3", 1},
    {0xD1F5, "GETR55", 1},
    {0xD1F3, "GETR52", 1},
    {0xD1E3, "GETR2", 1},
    {0xD1E2, "GETRCH", 1},
    {0xD1DF, "GETWCH", 1},
    {0xD1D3, "SETDRN", 1},
    {0xD1C6, "INCPTR", 1},
    {0xD1A3, "WRT0", 1},
    {0xD19D, "WRTBYT", 1},
    {0xD192, "RD4", 1},
    {0xD191, "RD3", 1},
    {0xD16A, "RD1", 1},
    {0xD164, "RD01", 1},
    {0xD156, "RDBYT", 1},
    {0xD151, "GETB1", 1},
    {0xD14D, "GETB2", 1},
    {0xD137, "GETBYT", 1},
    {0xD12F, "GETPRE", 1},
    {0xD125, "TYPFIL", 1},
    {0xD123, "FNDW20", 1},
    {0xD121, "FNDW15", 1},
    {0xD119, "FNDW10", 1},
    {0xD10F, "FNDW13", 1},
    {0xD107, "FNDWCH", 1},
    {0xD106, "FNDC30", 1},
    {0xD0F9, "FNDC25", 1},
    {0xD0F3, "FNDC20", 1},
    {0xD0EB, "FNDRCH", 1},
    {0xD0E8, "WRTC1", 1},
    {0xD0C9, "STRTIT", 1},
    {0xD0C7, "WRTBUF", 1},
    {0xD0C3, "RDBUF", 1},
    {0xD0B7, "STR1", 1},
    {0xD0AF, "STRDBL", 1},
    {0xD09B, "STRRD", 1},
    {0xD083, "NUMF2", 1},
    {0xD07D, "NUMF1", 1},
    {0xD075, "NFCALC", 1},
    {0xD042, "INITDR", 1},
    {0xD02C, "IT30", 1},
    {0xD024, "IT20", 1},
    {0xD00E, "ITRIAL", 1},
    {0xD00B, "ID20", 1},
    {0xD005, "INTDRV", 1},
    {0xCFFD, "PUTB1", 1},
    {0xCFF1, "PUTBYT", 1},
    {0xCFED, "L45", 1},
    {0xCFE8, "L50", 1},
    {0xCFD8, "L42", 1},
    {0xCFCE, "L46", 1},
    {0xCFC9, "L41", 1},
    {0xCFBF, "L40", 1},
    {0xCFB7, "PUT", 1},
    {0xCFAF, "PBYTE", 1},
    {0xCF9B, "PIBYTE", 1},
    {0xCF8C, "TGLBUF", 1},
    {0xCF8B, "DBS10", 1},
    {0xCF7B, "DBSET", 1},
    {0xCF76, "DBL30", 1},
    {0xCF6F, "DBL20", 1},
    {0xCF6C, "DBL15", 1},
    {0xCF66, "DBL10", 1},
    {0xCF5D, "DBL08", 1},
    {0xCF57, "DBL05", 1},
    {0xCF1E, "DBLBUF", 1},
    {0xCF1D, "LRUEXT", 1},
    {0xCF0D, "LRULP1", 1},
    {0xCF09, "LRUUPD", 1},
    {0xCEFC, "LRUILP", 1},
    {0xCEFA, "LRUINT", 1},
    {0xCEF0, "ADD100", 1},
    {0xCEED, "ADDRES", 1},
    {0xCEE6, "ACC200", 1},
    {0xCEE5, "ACCX2", 1},
    {0xCEE2, "ACCX4", 1},
    {0xCED9, "ZERRES", 1},
    {0xCED6, "DIV600", 1},
    {0xCEBF, "DIV500", 1},
    {0xCEB0, "DIV400", 1},
    {0xCEA3, "DIV300", 1},
    {0xCE89, "DIV200", 1},
    {0xCE87, "DIV150", 1},
    {0xCE71, "DIV120", 1},
    {0xCE6E, "DIV254", 1},
    {0xCE6D, "MUL400", 1},
    {0xCE57, "MUL200", 1},
    {0xCE50, "MUL100", 1},
    {0xCE4C, "MUL50", 1},
    {0xCE41, "MUL25", 1},
    {0xCE2C, "MULPLY", 1},
    {0xCE0E, "FNDREL", 1},
    {0xCDF5, "BLKTST", 1},
    {0xCDF2, "BKOTST", 1},
    {0xCDE5, "BT20", 1},
    {0xCDE0, "BT15", 1},
    {0xCDD2, "BUFTST", 1},
    {0xCDBD, "BLKPTR", 1},
    {0xCDBA, "BE10", 1},
    {0xCDA3, "BLKEXC", 1},
    {0xCD97, "UBLKWT", 1},
    {0xCD8C, "BW20", 1},
    {0xCD81, "BW10", 1},
    {0xCD73, "BLKWT", 1},
    {0xCD5F, "UBLKRD", 1},
    {0xCD56, "BLKRD", 1},
    {0xCD42, "BLKRD3", 1},
    {0xCD3C, "GETSIM", 1},
    {0xCD36, "BLKRD2", 1},
    {0xCD31, "BA40", 1},
    {0xCD2C, "BA30", 1},
    {0xCD1A, "BA20", 1},
    {0xCD19, "BA15", 1},
    {0xCD03, "BLKALC", 1},
    {0xCCF5, "BLKFRE", 1},
    {0xCCF2, "DECTAB", 1},
    {0xCCE4, "AH40", 1},
    {0xCCD7, "AH35", 1},
    {0xCCD0, "AH30", 1},
    {0xCCCA, "AH20", 1},
    {0xCCAB, "AH10", 1},
    {0xCCA1, "ASCHEX", 1},
    {0xCC92, "BP20", 1},
    {0xCC8B, "BP10", 1},
    {0xCC7C, "BP05", 1},
    {0xCC6F, "BLKPAR", 1},
    {0xCC42, "BLK60", 1},
    {0xCC38, "BLK50", 1},
    {0xCC30, "BLK40", 1},
    {0xCC2B, "BLK30", 1},
    {0xCC26, "BLK10", 1},
    {0xCC1B, "BLOCK", 1},
    {0xCBF1, "OB30", 1},
    {0xCBB8, "OB15", 1},
    {0xCBA5, "OB10", 1},
    {0xCBA0, "OB05", 1},
    {0xCB84, "OPNBLK", 1},
    {0xCB72, "USREXC", 1},
    {0xCB6C, "US10", 1},
    {0xCB63, "USRINT", 1},
    {0xCB5C, "USER", 1},
    {0xCB50, "MEMWRT", 1},
    {0xCB4B, "MEMERR", 1},
    {0xCB45, "M30", 1},
    {0xCB2B, "MRMULT", 1},
    {0xCB20, "MEMRD", 1},
    {0xCB1D, "MEMEX", 1},
    {0xCAF8, "MEM", 1},
    {0xCAF4, "CK30", 1},
    {0xCAEA, "CK25", 1},
    {0xCAE7, "CHKIO", 1},
    {0xCAE6, "CK20", 1},
    {0xCAD6, "CK10", 1},
    {0xCACC, "CHKIN", 1},
    {0xCA97, "RN10", 1},
    {0xCA88, "RENAME", 1},
    {0xCA53, "CYEXT", 1},
    {0xCA52, "GIB20", 1},
    {0xCA39, "GCBYTE", 1},
    {0xCA35, "GIBYTE", 1},
    {0xCA31, "OPIR10", 1},
    {0xC9FA, "OPIRFL", 1},
    {0xC9EA, "CY30", 1},
    {0xC9D8, "CY20", 1},
    {0xC9D5, "CY15", 1},
    {0xC9CE, "CY10A", 1},
    {0xC9B9, "CY10", 1},
    {0xC9A7, "CY", 1},
    {0xC9A1, "COP10", 1},
    {0xC987, "COP05", 1},
    {0xC982, "COP01", 1},
    {0xC952, "COPY", 1},
    {0xC932, "PUPS1", 1},
    {0xC928, "DX0020", 1},
    {0xC923, "DX0010", 1},
    {0xC90F, "DX0005", 1},
    {0xC90C, "DX0000", 1},
    {0xC8F0, "DSKCPY", 1},
    {0xC8EF, "FMT110", 1},
    {0xC8E0, "FMT105", 1},
    {0xC8C6, "FORMAT", 1},
    {0xC8B6, "DELDIR", 1},
    {0xC8AD, "DEL1", 1},
    {0xC894, "DEL2", 1},
    {0xC87D, "DELFIL", 1},
    {0xC872, "SC30", 1},
    {0xC86D, "SC25", 1},
    {0xC86B, "SC20", 1},
    {0xC855, "SC17", 1},
    {0xC835, "SC15", 1},
    {0xC823, "SCRTCH", 1},
    {0xC817, "FREMSG", 1},
    {0xC806, "MSGFRE", 1},
    {0xC7ED, "ND20", 1},
    {0xC7E5, "ND15", 1},
    {0xC7DC, "ND10", 1},
    {0xC7B7, "NEWDIR", 1},
    {0xC7B0, "BLKNB1", 1},
    {0xC7AC, "BLKNB", 1},
    {0xC7AB, "GN45", 1},
    {0xC7A7, "GN40", 1},
    {0xC783, "GN30", 1},
    {0xC773, "GN22", 1},
    {0xC76B, "GN20", 1},
    {0xC74A, "GN15", 1},
    {0xC73C, "GN14", 1},
    {0xC728, "GN12", 1},
    {0xC71B, "GN10", 1},
    {0xC70E, "GN051", 1},
    {0xC6FC, "GN050", 1},
    {0xC6F7, "GN05", 1},
    {0xC6DE, "GNSUB", 1},
    {0xC6CE, "GETNAM", 1},
    {0xC6C8, "FL10", 1},
    {0xC6AD, "FL05", 1},
    {0xC6A6, "FNDLMT", 1},
    {0xC6A5, "TR20", 1},
    {0xC697, "TR10", 1},
    {0xC688, "TRCMBF", 1},
    {0xC687, "TN20", 1},
    {0xC681, "TN10", 1},
    {0xC66E, "TRNAME", 1},
    {0xC669, "AUTO2", 1},
    {0xC65F, "AUTO1", 1},
    {0xC63D, "AUTOI", 1},
    {0xC62F, "SR50", 1},
    {0xC629, "SR40", 1},
    {0xC617, "SEARCH", 1},
    {0xC604, "SRRE", 1},
    {0xC5FB, "SR30", 1},
    {0xC5D7, "SR20", 1},
    {0xC5CA, "SR15", 1},
    {0xC5C4, "SR10", 1},
    {0xC5AC, "SRCHST", 1},
    {0xC5A6, "CC20", 1},
    {0xC59A, "CC15", 1},
    {0xC594, "CC10", 1},
    {0xC589, "CMPCHK", 1},
    {0xC55C, "CP42", 1},
    {0xC535, "CP40", 1},
    {0xC52B, "CP34", 1},
    {0xC51D, "CP33", 1},
    {0xC51B, "CP32", 1},
    {0xC50A, "CP30", 1},
    {0xC4FE, "CP20", 1},
    {0xC4EC, "CP10", 1},
    {0xC4E7, "CP05", 1},
    {0xC4E6, "CP02", 1},
    {0xC4D8, "COMPAR", 1},
    {0xC4D7, "FF40", 1},
    {0xC4C9, "FF30", 1},
    {0xC4BA, "FF25", 1},
    {0xC4B5, "FNDFIL", 1},
    {0xC4AA, "FF10", 1},
    {0xC49D, "FFST", 1},
    {0xC492, "FF15", 1},
    {0xC48B, "FFRE", 1},
    {0xC485, "LK30", 1},
    {0xC47E, "LK26", 1},
    {0xC475, "LK25", 1},
    {0xC470, "LK20", 1},
    {0xC462, "LK15", 1},
    {0xC45C, "LK10", 1},
    {0xC452, "LK05", 1},
    {0xC44F, "LOOKUP", 1},
    {0xC440, "SCHTBL", 1},
    {0xC43C, "OS45", 1},
    {0xC439, "OS70", 1},
    {0xC434, "OS60", 1},
    {0xC420, "OS50", 1},
    {0xC41B, "OS45", 1},
    {0xC400, "OS35", 1},
    {0xC3EF, "OS30", 1},
    {0xC3E8, "OS15", 1},
    {0xC3D5, "OS10", 1},
    {0xC3CA, "OPTSCH", 1},
    {0xC3C7, "T0V1", 1},
    {0xC3BD, "TST0V1", 1},
    {0xC3B8, "FS15", 1},
    {0xC3B0, "FS10", 1},
    {0xC398, "FS1SET", 1},
    {0xC38F, "TOGDRV", 1},
    {0xC388, "SA20", 1},
    {0xC383, "SA10", 1},
    {0xC370, "SA05", 1},
    {0xC368, "SETANY", 1},
    {0xC361, "SD50", 1},
    {0xC352, "SD40", 1},
    {0xC34F, "SD24", 1},
    {0xC34D, "SD22", 1},
    {0xC34C, "SD20", 1},
    {0xC33C, "SETDRV", 1},
    {0xC325, "AD10", 1},
    {0xC320, "ALLDRS", 1},
    {0xC312, "ONEDRV", 1},
    {0xC2DC, "CMDRST", 1},
    {0xC2CB, "CS08", 1},
    {0xC2CA, "CS07", 1},
    {0xC2B3, "CMDSET", 1},
    {0xC2B1, "PR40", 1},
    {0xC2A0, "PR35", 1},
    {0xC29E, "PR30", 1},
    {0xC299, "PR28", 1},
    {0xC283, "PR25", 1},
    {0xC280, "PR20", 1},
    {0xC26B, "PR10", 1},
    {0xC268, "PARSE", 1},
    {0xC260, "TC80", 1},
    {0xC254, "TC75", 1},
    {0xC24C, "TC70", 1},
    {0xC245, "TC60", 1},
    {0xC228, "TC50", 1},
    {0xC20A, "TC40", 1},
    {0xC200, "TC35", 1},
    {0xC1F8, "TC30", 1},
    {0xC1F3, "TC25", 1},
    {0xC1EE, "TAGCMD", 1},
    {0xC1E5, "PRSCLN", 1},
    {0xC1D1, "SIMPRS", 1},
    {0xC1C8, "CMDERR", 1},
    {0xC1BD, "CLRCB", 1},
    {0xC1AD, "SCREN1", 1},
    {0xC1A3, "SCREND", 1},
    {0xC194, "ENDCMD", 1},
    {0xC184, "PS30", 1},
    {0xC17A, "PS20", 1},
    {0xC16A, "PS10", 1},
    {0xC160, "PS5", 1},
    {0xC146, "PARSXQ", 1},
    {0xC12C, "ERRON", 1},
    {0xC123, "ERROFF", 1},
    {0xC118, "LEDSON", 1},
    {0xC100, "SETLDA", 1},
};

static const int _c1541_routine_count = sizeof(c1541_routines) / sizeof(c1541_routines[0]);

c1541_routine_t* _get_c1541_in_rom_routine(uint16_t addr) {
    for (int i = 0; i < _c1541_routine_count; i++) {
        if (addr >= c1541_routines[i].address) {
            return (c1541_routine_t*) &c1541_routines[i];
        }
    }
    return NULL;
}

static void _c1541_debug_out_processor_pc(uint tick, c1541_t* sys, uint64_t cpu_pins, uint64_t via1_pins) {
    if (cpu_pins & M6502_SYNC) {
        uint16_t cpu_pc = m6502_pc(&sys->cpu);
        c1541_routine_t* in_c1541_routine = _get_c1541_in_rom_routine(cpu_pc);
        uint16_t function_address = in_c1541_routine->address;
        uint16_t address_diff = cpu_pc - function_address;
        const char* function_name = in_c1541_routine->name;

        char iec_status[5];
        char local_iec_status[5];
        iec_get_status_text(sys->iec_bus, iec_status);
        iec_get_device_status_text(sys->iec_device, local_iec_status);

        uint8_t iec_lines = iec_get_signals(sys->iec_bus, sys->iec_device);
        via1_pins &= ~(M6522_PB0 | M6522_PB2 | M6522_PB7 | M6522_CA1);
        if (iec_lines & IECLINE_ATN) {
            via1_pins |= M6522_PB7;
            via1_pins |= M6522_CA1;
        }
        if (iec_lines & IECLINE_CLK) {
            via1_pins |= M6522_PB2;
        }
        if (iec_lines & IECLINE_DATA /*|| ((via1_pb & (1<<7)) >> 3) ^ (via1_pb & (1<<4))*/) {
            via1_pins |= M6522_PB0;
        }
        via1_pins = (via1_pins >> M6522_PIN_PB0) & 0xff;

        printf("tick:%10ld\taddr:%04x\tsys:c1541\tport-via1-b:%02x\tbus-iec:%s\tlocal-iec:%s\tlabel:%s+%x\n", tick, cpu_pc, via1_pins, iec_status, local_iec_status, function_name, address_diff);

/*
        if(cpu_pc==0xf4da) {
          if ((cpu_pc & 0xC000) == 0xC000) { // ROM
            _show_debug_trace('8', &sys->cpu, get_world_tick(),
                              sys->rom[(cpu_pc+0) & 0x3FFF], sys->rom[(cpu_pc+1) & 0x3FFF], sys->rom[(cpu_pc+2) & 0x3FFF]);
          } else if (cpu_pc < 0x0800) { // RAM
            _show_debug_trace('8', &sys->cpu, get_world_tick(),
                              sys->ram[(cpu_pc+0) & 0x07FF], sys->ram[(cpu_pc+1) & 0x07FF], sys->ram[(cpu_pc+2) & 0x07FF]);
          }
        }
*/
    }
}
